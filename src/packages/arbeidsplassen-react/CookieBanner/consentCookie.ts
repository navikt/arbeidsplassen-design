...
/** ---------- Zod schema ---------- */
const isoDatetime = z.string().refine((s) => !Number.isNaN(Date.parse(s)), { message: "Invalid ISO date" });
...
const isLegacy = (u: unknown): u is LegacyConsent => {
    ...
    const meta = o["meta"];
    const hasConsent = !!consent && typeof (consent as Record<string, unknown>).analytics === "boolean";
    const hasMeta =
        !!meta &&
...
/** ---------- Parse + normalisering ---------- */
export const parseConsentCookie = (raw: string | undefined): ConsentCookie | null => {
    if (!raw) return null;
...
export const writeConsent = (consent: ConsentCookie, days: number = CONSENT_COOKIE_MAX_AGE_DAYS): void => {
...
export const upsertConsent = (updater: (prev: ConsentCookie | null) => ConsentCookie): ConsentCookie => {
...
export const getAnalyticsConsent = (): boolean => readConsent()?.consent.analytics ?? false;

/** Hent nåværende surveys-samtykke (false hvis mangler/ugyldig). */
export const getSurveysConsent = (): boolean => readConsent()?.consent.skyraSurveys ?? false;

// --- Server/edge helpers ---
export const readConsentFromCookieHeader = (cookieHeader: string | null | undefined): ConsentCookie | null => {
...
export const makeSetCookieHeader = (
    ...
    const secureFlag = typeof opts?.secure === "boolean" ? opts.secure : mustSecure ? true : isHttps;
