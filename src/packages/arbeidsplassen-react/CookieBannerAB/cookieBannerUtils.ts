...
const isObject = (v: unknown): v is Readonly<Record<string, unknown>> => typeof v === "object" && v !== null;
...
function validateConsentDataLoose(input: unknown): input is ConsentData {
    ...
    if (typeof m.createdAt !== "string" || !isValidISOString(m.createdAt)) return false;
    if (typeof m.updatedAt !== "string" || !isValidISOString(m.updatedAt)) return false;
    if (!isNumber(m.version) || !Number.isInteger(m.version) || (m.version as number) < 0) return false;
...
export function setCookie(value: ConsentData, days: number = CONSENT_COOKIE_MAX_AGE_DAYS): void {
...
export function updateConsent(
    ...
    const nextLegacy: ConsentData = normalizeLegacy({
        consent: {
            ...
            skyraSurveys: partial.consent?.skyraSurveys ?? currentLegacy.consent.skyraSurveys,
        },
